...
import { processOneBrowserJob } from './worker';
...
async function main(): Promise<void> {
  ...
  const registry = buildStepRegistry([chatgptBrowserStepMeta], 'browser');

  // eslint-disable-next-line no-console
  console.log(
    `worker-browser started (redis=${redisUrl ? 'yes' : 'in-memory'}, pg=${process.env.DATABASE_URL ? 'yes' : 'in-memory'})`,
  );

  // eslint-disable-next-line no-constant-condition
  while (true) {
    try {
      const handled = await processOneBrowserJob({
        jobQueue,
        orchestrator,
        proxyPool,
        store,
      ...
      // eslint-disable-next-line no-console
      console.error('[worker-browser] unexpected loop error:', err);
      await new Promise((r) => setTimeout(r, POLL_MS));
    }
  }
}
