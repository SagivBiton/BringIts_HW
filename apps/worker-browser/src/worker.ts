...
/** Process one browser-queue job; returns whether a job was handled. */
export async function processOneBrowserJob(deps: BrowserWorkerDeps): Promise<boolean> {
  const job = await deps.jobQueue.dequeue('browser');
  if (!job) return false;
  ...
  try {
    ...
    console.error(`[worker-browser] job ${job.request_id} failed:`, err);
    try {
      await deps.proxyPool.release(job.lease_id, 'neutral');
      ...
      // eslint-disable-next-line no-console
      console.error(
        `[worker-browser] cleanup failed for ${job.request_id}:`,
        cleanupErr,
      );
    }
  }
